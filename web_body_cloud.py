# web_body_cloud.py  DeepSeek 版  一键复制
...
def ask_gpt(age, sex, scores):
    ...
    prompt = f"""你是一位中医健康管理师，请根据以下问卷得分（满分45）给出：1. 体质类型（单选：平和质/气虚质/阳虚质/痰湿质/湿热质/血瘀质/气郁质/特禀质）2. 一句话解释（≤50字）3. 三条可执行养生建议（食疗/运动/作息）输出 JSON 格式：{{"体质":"","解释":"","建议":{{"食疗":"","运动":"","作息":""}}}} 年龄：{age} 性别：{sex} 问卷得分：{scores} 总分：{sum(scores)}"""
    data = {"model": "deepseek-chat", "temperature": 0.3, "messages": [{"role": "user", "content": prompt}]}
